...
export default function CoordinatorLayoutPage() {
  const navigate = useNavigate();
  const { user, logout, hasPermission } = useAuth();
  const [now, setNow] = useState(new Date());
  const [message, setMessage] = useState(null);
  const [notifications, setNotifications] = useState([
    { id: 1, title: '3 applications need review', time: 'Just now', unread: true, type: 'urgent' },
    { id: 2, title: 'Weekly reports submitted', time: '2h ago', unread: true, type: 'info' },
    { id: 3, title: 'New internship post pending', time: 'Yesterday', unread: false, type: 'info' },
  ]);
  ...
  const nav = useMemo(() => {
    const items = [
      { to: '/dashboard/coordinator', label: 'Dashboard', icon: '📊', permission: 'system.dashboard.view' },
      { to: '/dashboard/coordinator/registration', label: 'User Registration', icon: '➕', permission: 'users.create' },
      { to: '/dashboard/coordinator/users', label: 'User Management', icon: '👥', permission: 'users.viewAny' },
      { to: '/dashboard/coordinator/assignments', label: 'Assignments', icon: '🧩', permission: 'assignments.examiner.assign' },
      { to: '/dashboard/coordinator/reports', label: 'Reports', icon: '📄', permission: 'reports.custom.generate' },
      { to: '/dashboard/coordinator/approvals', label: 'Approvals', icon: '✅', permission: 'applications.review' },
      { to: '/dashboard/coordinator/settings', label: 'Settings', icon: '⚙️', permission: 'settings.department.manage' },
    ];

    // Keep items visible, but mark locked if permission is missing.
    return items.map((i) => ({
      ...i,
      locked: !!i.permission && !hasPermission(i.permission),
    }));
  }, [hasPermission]);
  ...
  const registerUser = async (payload) => {
    ...
    // Backend expects name/email/role. Extra fields are ignored unless supported.
    ...
    <div className="coord-shell">
      <aside className="coord-sidebar">
        <div className="coord-brand">
          <div className="coord-brand-icon">🎓</div>
          <div>
            <div className="coord-brand-title">ARU IMS</div>
            <div className="coord-brand-subtitle">Department Admin Portal</div>
          </div>
        </div>
            ...
            item.locked ? (
              <button
                ...
                onClick={() =>
                  setMessage({
                    type: 'info',
                    text: `Not allowed by authority matrix: ${item.permission}`,
                  })
                }
              >
        ...
        <div className="coord-sidebar-footer">
          <div className="coord-user">
            ...
            <div className="coord-user-meta">
              ...
              <div className="coord-user-sub">Department Admin</div>
            </div>
          </div>
      ...
      <main className="coord-main">
        <header className="coord-topbar">
          <div className="coord-topbar-left">
            <div className="coord-topbar-title">Department Operations</div>
            <div className="coord-topbar-subtitle">{formatClock(now)}</div>
          </div>

          <div className="coord-topbar-right">
            <Dropdown align="end">
              <Dropdown.Toggle className="coord-bell" variant="light" id="coord-notifications">
                🔔
                {unreadCount > 0 ? (
              ...
              <Dropdown.Menu className="coord-dropdown">
                ...
                <Dropdown.Divider />
                <Dropdown.Item onClick={() => navigate('/dashboard/coordinator/approvals')}>
                  Go to approvals
                </Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>
