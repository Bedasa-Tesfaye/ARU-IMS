...
const types = [
  { id: 'student', label: 'Student', icon: '🎓', blurb: 'Single or bulk intake' },
  { id: 'company', label: 'Company', icon: '🏢', blurb: 'Industry partners' },
  { id: 'examiner', label: 'Examiner', icon: '👨‍🏫', blurb: 'Assessment staff' },
    ...
    <div className="sa-registration-hub">
      <header className="sa-reg-hub-header">
        <div>
          ...
          <p className="sa-reg-hub-sub">Choose who you are onboarding — the form slides in with guided steps.</p>
        </div>
      </header>

      <div className="sa-reg-progress" aria-label="Registration progress">
        <div className={`sa-reg-progress-step ${step1 ? 'done' : 'current'}`}>
          <span className="sa-reg-progress-num">1</span>
          <span>Type</span>
        </div>
        <div className="sa-reg-progress-line" />
        <div className={`sa-reg-progress-step ${step2 ? 'current' : 'pending'}`}>
          <span className="sa-reg-progress-num">2</span>
          <span>Details</span>
        </div>
        <div className="sa-reg-progress-line" />
        <div className={`sa-reg-progress-step ${step3 ? 'current' : 'pending'}`}>
          <span className="sa-reg-progress-num">3</span>
          <span>Submit</span>
        </div>
      </div>

      <div className="sa-reg-type-grid" role="tablist" aria-label="Registration type">
        {types.map((t) => (
          <button
            key={t.id}
            type="button"
      ...
      <div className="sa-reg-form-shell" key={activeSection}>
        ...
        {activeSection === 'company' && (
          <CompanyRegistration onRegister={onRegister} isSubmitting={isSubmitting} />
        )}
        {activeSection === 'examiner' && (
          <ExaminerRegistration departments={departments} onRegister={onRegister} isSubmitting={isSubmitting} />
        )}
        {activeSection === 'advisor' && (
          <AdvisorRegistration departments={departments} onRegister={onRegister} isSubmitting={isSubmitting} />
        )}
        {!['student', 'company', 'examiner', 'advisor'].includes(activeSection) && (
      ...
      {currentMeta && (
        <p className="sa-reg-active-hint">
          Active: <strong>{currentMeta.icon}</strong> {currentMeta.label}
        </p>
      )}
    </div>
  );
