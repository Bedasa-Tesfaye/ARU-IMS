...
const UserList = ({
  ...
  const handleBulkAction = (action) => {
    ...
    const confirmMsg = `Are you sure you want to ${action} ${selectedUsers.size} user(s)?`;
      ...
      {selectedUsers.size > 0 && (
        <div className="bulk-actions-bar">
          ...
          <div className="bulk-buttons">
            <button type="button" className="bulk-suspend" onClick={() => handleBulkAction('suspend')}>
              ⏸️ Suspend Selected
            </button>
            <button type="button" className="bulk-delete" onClick={() => handleBulkAction('delete')}>
              🗑️ Delete Selected
            </button>
          </div>
        </div>
      ...
      <div className="table-responsive">
        <table className="user-table">
          <thead>
            <tr>
              <th>
                <input type="checkbox" checked={selectAll && users.length > 0} onChange={handleSelectAll} />
              </th>
              <th>User</th>
              <th>Contact</th>
              <th>Role</th>
            ...
            {users.length === 0 ? (
              ...
              return (
                <tr key={user.id} className="user-row">
                  ...
                  <td>
                    <span className={`status-badge ${user.status}`} style={{ background: statusInfo.bg, color: statusInfo.color }}>
